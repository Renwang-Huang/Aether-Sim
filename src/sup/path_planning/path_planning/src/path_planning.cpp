...
public:
	...
	{
		if(prev_goal[0] != x || prev_goal[1] != y || prev_goal[2] != z)//判断目标点是否与上次的目标点相同
		{
	...
	// Constructor 构造函数 实例化类时会自动调用
	planner(void)
	{
		Quadcopter = std::shared_ptr<fcl::CollisionGeometry>(new fcl::Box(0.3, 0.3, 0.1));//定义四旋翼大小
...
void octomapCallback(const octomap_msgs::Octomap::ConstPtr &msg, planner* planner_ptr)
{
	...
	planner_ptr->replan();//当有新的octomap地图更新时会再次进行路径规划
}
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber octree_sub = n.subscribe<octomap_msgs::Octomap>("/octomap_binary", 1, boost::bind(&octomapCallback, _1, &planner_object));//订阅octomap
	ros::Subscriber odom_sub = n.subscribe<nav_msgs::Odometry>("/mavros/local_position/odom", 1, boost::bind(&odomCb, _1, &planner_object));//订阅无人机初始位置
	ros::Subscriber goal_sub = n.subscribe<geometry_msgs::PointStamped>("/clicked_point", 1, boost::bind(&goalCb, _1, &planner_object));//订阅无人机的目标位置
